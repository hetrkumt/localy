...
# -------------------------------------------------------------------------
# 2) Lambda 실행 Role 보강 — Secrets / Logs / VPC ENI
# -------------------------------------------------------------------------
data "aws_iam_policy_document" "chatops_lambda_assume" {
  statement {
...
data "aws_iam_policy_document" "chatops_lambda_runtime" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:logs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${local.alarm_pipeline_lambda_fn_name}:*",
    ]
  }
...
resource "aws_iam_role" "alarm_pipeline_lambda" {
  ...
  assume_role_policy = data.aws_iam_policy_document.chatops_lambda_assume.json
...
# -------------------------------------------------------------------------
# 4) Lambda placeholder artifact (소스 코드 격리 완료)
# -------------------------------------------------------------------------
...
# ========================================================================
# ChatOps JIT Auth Lambda — Slack Interactivity 신원 검증
...
data "aws_iam_policy_document" "chatops_jit_auth_lambda_runtime" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:logs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${local.chatops_jit_auth_lambda_fn_name}:*",
    ]
  }
