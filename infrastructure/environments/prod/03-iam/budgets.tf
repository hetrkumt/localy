...
#   Phase 1: Loki 荑쇰━ 鍮꾩슜 ??100% ??S3 GetObject Deny
#   Phase 2: ?뚮엺 ?뚯씠?꾨씪????90% ??sns:Publish Deny (MANUAL approval)
# ========================================================================
...
resource "aws_iam_policy" "loki_budget_circuit_breaker_deny" {
  name        = "${module.global.env_name}-loki-budget-circuit-breaker-deny"
...
resource "aws_iam_policy" "alarm_pipeline_budget_circuit_breaker_deny" {
  name        = "${module.global.env_name}-alarm-pipeline-budget-circuit-breaker-deny"
...
# -------------------------------------------------------------------------
# Budgets Action Execution Role (Trust + IAM attach 沅뚰븳 ??Phase 1 + Phase 2 怨듭슜)
# -------------------------------------------------------------------------
...
resource "aws_iam_role" "budgets_action_execution" {
  name               = "${module.global.env_name}-budgets-action-execution-role"
...
resource "aws_iam_role_policy" "budgets_action_execution" {
  name   = "${module.global.env_name}-budgets-action-execution-policy"
...
# -------------------------------------------------------------------------
# Phase 1 ??Loki ?쒓렇 湲곕컲 ?붽컙 ?덉궛 + 100% Kill Switch
# -------------------------------------------------------------------------
resource "aws_budgets_budget" "loki_daily_cost" {
  name         = "${module.global.env_name}-loki-daily-cost-circuit-breaker"
  budget_type  = "COST"
  limit_amount = "1500"
  limit_unit   = "USD"
  time_unit    = "MONTHLY"
...
# -------------------------------------------------------------------------
# Phase 2 ????$5 ?뚮엺 ?뚯씠?꾨씪???쒓렇 湲곕컲 ?덉궛 + 90% Kill Switch (MANUAL)
# -------------------------------------------------------------------------
resource "aws_budgets_budget" "alarm_pipeline_daily_cost" {
  name         = "${module.global.env_name}-alarm-pipeline-daily-cost-circuit-breaker"
  budget_type  = "COST"
  limit_amount = "5"
  limit_unit   = "USD"
  time_unit    = "DAILY"
