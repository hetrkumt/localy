...
data "aws_lambda_function" "chatops_jit_auth" {
  function_name = "${module.global.env_name}-chatops-jit-auth"
}

resource "aws_apigatewayv2_api" "chatops_jit" {
  name          = "${module.global.env_name}-chatops-jit-api"
  protocol_type = "HTTP"
...
resource "aws_apigatewayv2_stage" "chatops_jit" {
  ...
  tags = {
    Name        = "${module.global.env_name}-chatops-jit-api-stage"
    Environment = module.global.env_name
    ManagedBy   = "terraform"
    Purpose     = "chatops-jit-auth"
  }
...
resource "aws_apigatewayv2_integration" "chatops_jit_auth" {
  ...
  integration_uri        = data.aws_lambda_function.chatops_jit_auth.invoke_arn
...
resource "aws_lambda_permission" "chatops_jit_auth_apigw" {
  ...
  function_name = data.aws_lambda_function.chatops_jit_auth.function_name
...
resource "aws_acm_certificate" "chatops_jit_cert" {
  domain_name       = "api.${module.global.base_domain}"
